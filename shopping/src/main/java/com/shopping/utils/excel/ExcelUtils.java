...
import com.shopping.domain.excel.ExcelHead;
...
public class ExcelUtils {
     ...
     * @throws Exception
     */
    public static <T> List<T> readExcelToEntity(Class<T> classzz, InputStream in, String fileName, List<ExcelHead> excelHeads, int sheetIndex) throws Exception {
        checkFile(fileName);    //是否EXCEL文件
        Workbook workbook = getWorkBoot(in, fileName); //兼容新老版本
        List<T> excelForBeans = readExcel(classzz, workbook, excelHeads,sheetIndex);  //解析Excel
     ...
     * @throws Exception
     */
    public static <T> List<T> readExcelToEntity(Class<T> classzz, InputStream in, String fileName,int sheetIndex) throws Exception {
        return readExcelToEntity(classzz, in, fileName, null,sheetIndex);
    }
